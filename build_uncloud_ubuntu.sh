...
build_architecture() {
    ...
    for dist in "${arr[@]}"; do
        ...
        id="$(docker create "uncloud-ubuntu-$dist-$build_arch")"
        ...
        if ! docker build . -f uncloudd_Dockerfile.ubu -t "uncloudd-ubuntu-$dist-$build_arch" \
            --build-arg uncloud_VERSION="$uncloud_VERSION" \
            --build-arg UBUNTU_DIST="$dist" \
            --build-arg BUILD_VERSION="$BUILD_VERSION" \
            --build-arg FULL_VERSION="$FULL_VERSION" \
        ...
        id="$(docker create "uncloudd-ubuntu-$dist-$build_arch")"
